...
class SuppliersView(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        # Barra de filtrado dinámico (RF2.2)
        filter_bar = ctk.CTkFrame(self, fg_color="transparent", height=45)
        filter_bar.pack(fill="x", padx=20, pady=(10, 0))

        ctk.CTkLabel(
            filter_bar, text="🔍 Filtrar por Tipo de Producto (Categoría):",
    ...
    def load_suppliers(self):
        # Rellenar filtro si es la primera vez
        cats = SupplierModel.get_categories()
        cat_names = ["[Todas las Categorías]"] + [c['nombre'] for c in cats]
        if len(self.filter_category.cget("values")) <= 1 and len(cat_names) > 1:
            self.filter_category.configure(values=cat_names)
        ...
        for s in suppliers:
            # Filtrado por tipo de producto (RF2.2)
            if selected_cat != "[Todas las Categorías]" and s['categoria_nombre'] != selected_cat:
                continue
    ...
    def open_new_supplier_dialog(self):
        ...
        e_dir = ctk.CTkEntry(dialog, placeholder_text="Dirección del Proveedor", width=350)
        e_dir.pack(pady=6)
