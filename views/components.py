...
class HeaderFrame(ctk.CTkFrame):
    """ Header Superior Corporativo Rediseñado (SaaS Style) """
    def __init__(self, master, title="PANEL DE CONTROL GENERAL", **kwargs):
        ...
        logo_box = ctk.CTkFrame(left_box, fg_color="#0F172A", corner_radius=10, border_width=1, border_color="#334155")
...
class StatusChip(ctk.CTkLabel):
    """ Badge / Tag de Estado Estilizado tipo Chip SaaS """
    def __init__(self, master, text, status_type="success", **kwargs):
        color_map = {
            "success": (COLOR_SUCCESS, "#064E3B"),
            "warning": (COLOR_WARNING, "#451A03"),
            "danger": (COLOR_DANGER, "#451212"),
            "accent": (COLOR_ACCENT, "#0F172A"),
            "purple": (COLOR_PURPLE, "#2E1065"),
            "muted": (COLOR_TEXT_MUTED, "#1E293B")
        }
        fg_text, bg_fill = color_map.get(status_type, (COLOR_ACCENT, "#0F172A"))
        super().__init__(
            master, text=text,
            font=ctk.CTkFont(family="Segoe UI", size=11, weight="bold"),
...
class MetricCard(ctk.CTkFrame):
    """ Tarjeta de Métrica KPI SaaS con Tendencia e Ícono """
    def __init__(self, master, title, value, icon="📊", subtext="", trend="▲ +12.5%", accent_color=COLOR_ACCENT, **kwargs):
        super().__init__(master, fg_color=COLOR_BG_CARD, corner_radius=14, border_width=1, border_color="#334155", **kwargs)
        ...
        icon_box = ctk.CTkFrame(top_row, fg_color="#0F172A", corner_radius=8, width=34, height=34)
        ...
        if trend:
            trend_badge = ctk.CTkLabel(
                ...
                text_color=COLOR_SUCCESS if "▲" in trend else (COLOR_DANGER if "▼" in trend else COLOR_ACCENT),
                fg_color="#0F172A", corner_radius=4, padx=6, pady=2
            )
...
class PrimaryButton(ctk.CTkButton):
    """ Botón Principal Estilizado (Electric Blue) """
...
class AccentButton(ctk.CTkButton):
    """ Botón Secundario / Acento Estilizado (Cyan Neon) """
    def __init__(self, master, text, command=None, icon=None, **kwargs):
        full_text = f"{icon} {text}" if icon else text
        btn_height = kwargs.pop("height", 38)
        super().__init__(
            ...
            text_color=COLOR_ACCENT, border_width=1, border_color="#38BDF8", corner_radius=8,
            font=ctk.CTkFont(family="Segoe UI", size=12, weight="bold"),
            height=btn_height, **kwargs
        )
...
class CardFrame(ctk.CTkFrame):
    """ Contenedor Tarjeta Glassmorphic Reutilizable """
